...
session_start();
$_SESSION['newproblem']=$_GET['problem'];
$_SESSION['test']=$_GET['test'];

echo $_SESSION['newproblem'];
...
function pyTest($options, $content) {
 ...
 if ( !userIsAdmin()) 
    return __t("<br/><br/>Nemate pravo pristupa stranici <b>Testovi</b>.");

  echo '<script type="text/javascript">
function validate(){
var level=document.getElementById("level").value;
if(level=="")alert("Morate odabrati nivo!");
else document.studenttest.submit();
}
...
var test_id = $("select#test option:selected").attr("value");
if(test_id != "")changeTestsProblems(test_id);
$("select#test").change(function(){
var test_id = $("select#test option:selected").attr("value");
changeTestsProblems(test_id);
});
});
...
error: function(jxhr){alert("Sacekajte učitavanje testova")}
});
}

function changeTestsProblems(test_id)
{
$.ajax({
type: "GET",
async: false,
  ...
  if ($cstudents>0) {
    ...
    $preamble = 
      "<div class='progress-selector'>
       <form method='get' id='studenttest'><table style='border:none'><tr><td>".__t("Želite li da pratite rad svog studenta?").'</td><td>';
    $options = array();
    ...
    $preamble .= "<tr><td>".__t("Želite li testove i zadatke za određeni nivo?")."</td><td>";
    $options = array();
    ...
    $preamble .= "</td></tr><tr><td colspan='3' style='text-align:center'><input style='width: 25%' type='submit' value='".__t('Potvrdi')."' onclick='validate();'/></tr></td></table></form></div>";
    echo $preamble;
...
AND wp_pb_lessons.is_test=1
    ...
    foreach (getStudentsByDate(false,$date) as $stu) {
      ...
      $studentTable .= '<a class="open-same-window" href="?user=' . $stu .'&datepicker='.$_GET['datepicker'].'&level='.$_GET['level'].'&test='.$_GET['test'].'&problem=' . $gp . '">';
...
foreach (getStudentsByDate(false,$date) as $stu) {
      ...
      $studentTable .= '<td>';
      if($_GET['test']==""){
      $studentTable .= '<a class="open-same-window" href="?user=' . $stu .'&datepicker='.$_GET['datepicker'].'&level='.$_GET['level'].'&test=&problem=">';
      $studentTable .= userString($stu);
      $studentTable .= '</a></td>';}
else{
  $studentTable .= '<a class="open-same-window" href="?user=' . $stu .'&datepicker='.$_GET['datepicker'].'&level='.$_GET['level'].'&test='.$_GET['test'].'&problem=">';
      $studentTable .= userString($stu);
      $studentTable .= '</a></td>';
}
     ...
     if($_GET['test']==""){
      foreach($testbylevel as $test)
      {
          ...
          $studentTable .="<a href=".get_page_link($test['id'])." target='_blank'>". $test['ordering'].":".$test['title']."</a> &#8658<b>Poeni:";

$points=$wpdb->get_var("SELECT SUM(points) AS Total 
FROM wp_pb_problems,wp_pb_submissions
WHERE postid=".$test['id']."
AND wp_pb_problems.slug=wp_pb_submissions.problem
AND wp_pb_submissions.result='Y'
AND wp_pb_submissions.userid=$stu");
$studentTable.=($points>0)?$points:'0';
$studentTable.="</b><br/>";
      }
}
else 
...
AND wp_pb_lessons.is_test=1
 ...
 {
   return __t("<br/><br/>Trenutno nemate dodeljenih studenata.");
 }
}
