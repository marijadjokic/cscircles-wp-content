...
function dbEntireTestHistory($limit, $sortname, $sortorder, $req=NULL) {
  global $db_query_info;
  $db_query_info = array();
   if ($req == NULL) $req = $_REQUEST;
   $db_query_info['type'] = 'entire-test-history';

   $user = getSoft($req, "user", "");   
   $problem = getSoft($req, "problemhash", ""); 
   $level = getSoft($req, "level", ""); 
   $test= getSoft($req, "test", ""); 
...
if(strpos($_SERVER["SCRIPT_FILENAME"], '/db-entire-test-history.php')!=FALSE) {
